...
bool8 test_prioritysort() {
  ...
  pid32 i = NPROC - 1;
  enqueue(i, q, NPROC - 1);
  for(i = (i - 7 + NPROC) % NPROC; i != NPROC - 1; i = (i - 7 + NPROC) % NPROC)  {
    enqueue(i, q, i);
  }
...
int n = N_DEADLOCK_ORIG;

pid32 *childpids;
lid32 *lockids;


void deadlock_worker(int id) {
  
  childpids[id] = currpid;
  acquire(lockids[id]);
...
// unit test deadlock recovery
// as laid out in README.md
bool8 test_deadlock_recovery() {
  
  //implement deadlock recovery test
  childpids = malloc(sizeof(pid32) * N_DEADLOCK_ORIG);
  lockids = malloc(sizeof(lid32) * N_DEADLOCK_ORIG);
  
  int i;
  // create ready queue of 5 proceses
  for (i=0;i<N_DEADLOCK_ORIG;i++){
    childpids[i] = -1;
    lockids[i] = lock_create();
    ready(create((void*) deadlock_worker, INITSTK, 15, "Deadlock Test Worker", 1, i), FALSE);
  }

  // wait for all child pids to have the status PR_FREE
  for (i=0;i<N_DEADLOCK_ORIG;i++){
    while(childpids[i] == -1 ? TRUE : proctab[childpids[i]].prstate != PR_FREE) resched();
  }
  
  // Clean up
  free(childpids, sizeof(pid32) * N_DEADLOCK_ORIG);
  free(lockids, sizeof(lid32) * N_DEADLOCK_ORIG);

  // The ending value of the counter should be `n-1`
  return (n == N_DEADLOCK_ORIG - 1);
}

void undercover(uint32 id){

}

void agent(uint32 id){
...
int runtests()
{
  ...
  pass &= runtest(&test_deadlock_recovery, "deadlock recovery");
  kprintf("----------------------------\n");
